...
<body>
  ${cvMarkup}
    ...
    if (tplId === 'meridian' || tplId === 'sterling') {
      return `<!doctype html>
<html>
<head>
  <meta charset="utf-8" />
  ...
  <style>
    ...
    * { box-sizing: border-box; -webkit-print-color-adjust: exact; print-color-adjust: exact; }
    ...
    #cv-print-area { width: 210mm; max-width: 210mm; margin: 0; padding: 0; background: white; }
    #cv-print-area > div > div:not([data-screen-doc]):not([data-renderer-page]) { display: none !important; }
    [data-measure-pass] { display: none !important; }
    [data-renderer-page] { width: 210mm !important; height: 297mm !important; margin: 0 !important; overflow: hidden !important; page-break-after: always; break-after: page; }
    [data-renderer-page]:last-child { page-break-after: auto; break-after: auto; }
    [data-renderer-page] > div { min-height: 0; overflow: hidden; }
    [data-renderer-continuation] ul { list-style-type: none !important; padding-left: 16px !important; }
    [data-renderer-continuation] li { list-style-type: none !important; }
  </style>
</head>
<body>
  ${cvMarkup}
  <script>
    (async function () {
      try {
        ...
        const root = document.getElementById('cv-print-area');
        const wrapper = root && root.firstElementChild;
        const screen = wrapper && wrapper.querySelector('[data-screen-doc]');
        const frame = screen && screen.firstElementChild;
        if (!root || !wrapper || !screen || !frame) return;

        const mainBlocks = Array.from(frame.querySelectorAll('[data-bk]'));
        const sideBlocks = Array.from(frame.querySelectorAll('[data-sbk]'));
        const mainSource = mainBlocks[0] && mainBlocks[0].parentElement;
        const sideSource = sideBlocks[0] && sideBlocks[0].parentElement;
        if (!mainSource || !sideSource) return;

        const headerNodes = Array.from(mainSource.children).filter(function (node) { return !node.hasAttribute('data-bk'); });
        const mainChildIndex = Array.from(frame.children).indexOf(mainSource);
        const sideChildIndex = Array.from(frame.children).indexOf(sideSource);
        const guard = 10;
        const pages = [];

        function makePage(first) {
          const page = frame.cloneNode(false);
          page.setAttribute('data-renderer-page', '');
          const columns = Array.from(frame.children).map(function (column) { return column.cloneNode(false); });
          columns.forEach(function (column) { page.appendChild(column); });
          const main = columns[mainChildIndex];
          const side = columns[sideChildIndex];
          if (first) headerNodes.forEach(function (node) { main.appendChild(node.cloneNode(true)); });
          wrapper.appendChild(page);
          const entry = { page: page, main: main, side: side };
          pages.push(entry);
          return entry;
        }

        function fits(column, node) {
          column.appendChild(node);
          const columnRect = column.getBoundingClientRect();
          const nodeRect = node.getBoundingClientRect();
          if (nodeRect.bottom <= columnRect.bottom - guard) return true;
          node.remove();
          return false;
        }

        function fitsPair(column, first, second) {
          const firstCopy = first.cloneNode(true);
          const secondCopy = second.cloneNode(true);
          column.appendChild(firstCopy);
          column.appendChild(secondCopy);
          const fitsBoth = secondCopy.getBoundingClientRect().bottom <= column.getBoundingClientRect().bottom - guard;
          firstCopy.remove();
          secondCopy.remove();
          return fitsBoth;
        }

        function splitBullet(source, column) {
          const whole = source.cloneNode(true);
          column.appendChild(whole);
          const text = whole.querySelector('[data-renderer-splittable]');
          const textNode = text && text.firstChild;
          if (!text || !textNode || textNode.nodeType !== Node.TEXT_NODE) { whole.remove(); return null; }
          const words = text.textContent.match(/\\S+\\s*/g) || [];
          const range = document.createRange();
          const boundary = column.getBoundingClientRect().bottom - guard;
          let offset = 0;
          let end = 0;
          const lines = new Set();
          words.forEach(function (word) {
            ...
            if (rect.bottom <= boundary) { end = offset; lines.add(Math.round(rect.top)); }
          });
          whole.remove();
          const value = text.textContent;
          if (!end || end >= value.length || lines.size < 2) return null;
          const first = source.cloneNode(true);
          const rest = source.cloneNode(true);
          first.querySelector('[data-renderer-splittable]').textContent = value.slice(0, end).trim();
          rest.querySelector('[data-renderer-splittable]').textContent = value.slice(end).trim();
          rest.setAttribute('data-renderer-continuation', '');
          column.appendChild(first);
          return rest;
        }

        let current = makePage(true);
        for (let index = 0; index < mainBlocks.length; index++) {
          const source = mainBlocks[index];
          const next = mainBlocks[index + 1];
          const isHeading = /-h$/.test(source.getAttribute('data-bk') || '');
          const hasContent = current.main.querySelector('[data-bk]');
          if (isHeading && next && hasContent && !fitsPair(current.main, source, next)) current = makePage(false);
          const node = source.cloneNode(true);
          if (fits(current.main, node)) continue;
          const rest = splitBullet(source, current.main);
          current = makePage(false);
          if (rest) {
            if (!fits(current.main, rest)) current.main.appendChild(rest);
            continue;
          }
          if (!fits(current.main, source.cloneNode(true))) current.main.appendChild(source.cloneNode(true));
        }

        let sidePage = 0;
        sideBlocks.forEach(function (source) {
          while (true) {
            if (!pages[sidePage]) makePage(false);
            if (fits(pages[sidePage].side, source.cloneNode(true))) return;
            sidePage++;
          }
        });
        screen.remove();
      } finally {
        window.__swiftcvRendererReady = true;
      }
    })();
  </script>
</body>
