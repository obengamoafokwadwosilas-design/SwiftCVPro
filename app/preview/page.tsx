...
// ══════════════════════════════════════════════════════
// LIVE PREVIEW — matches actual PDF output
// ══════════════════════════════════════════════════════

const ICON_EMAIL = `<svg style="display:inline;vertical-align:middle;marginRight:4px" width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><rect x="2" y="4" width="20" height="16" rx="2"/><polyline points="22,6 12,13 2,6"/></svg>`
const ICON_PHONE = `<svg style="display:inline;vertical-align:middle;marginRight:4px" width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07A19.5 19.5 0 0 1 4.11 10 19.79 19.79 0 0 1 1.04 1.54 2 2 0 0 1 3 0h3a2 2 0 0 1 2 1.72c.127.96.361 1.903.7 2.81a2 2 0 0 1-.45 2.11L7.09 7.91a16 16 0 0 0 6 6l1.27-1.27a2 2 0 0 1 2.11-.45c.907.339 1.85.573 2.81.7A2 2 0 0 1 22 16.92z"/></svg>`
const ICON_PIN = `<svg style="display:inline;vertical-align:middle;marginRight:4px" width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0 1 18 0z"/><circle cx="12" cy="10" r="3"/></svg>`

function ContactLine({ cv, color = '#5a5a5a' }: { cv: GeneratedCV; color?: string }) {
  const items = [
    cv.email && <span key="e" style={{ color, marginRight: 18, fontSize: '10pt', display: 'inline-flex', alignItems: 'center', gap: 4, whiteSpace: 'nowrap' }}>
    ...
    cv.phone && <span key="p" style={{ color, marginRight: 18, fontSize: '10pt', display: 'inline-flex', alignItems: 'center', gap: 4, whiteSpace: 'nowrap' }}>
    ...
    cv.location && <span key="l" style={{ color, marginRight: 18, fontSize: '10pt', display: 'inline-flex', alignItems: 'center', gap: 4, whiteSpace: 'nowrap' }}>
    ...
    cv.linkedin && <span key="li" style={{ color, marginRight: 18, fontSize: '10pt', display: 'inline-flex', alignItems: 'center', gap: 4, whiteSpace: 'nowrap' }}>
  ...
  return (
    <div style={{ fontFamily: "'Source Sans 3', 'Helvetica Neue', sans-serif", fontSize: '10.5pt', lineHeight: 1.65, color: '#1a1a1a', background: '#ffffff', padding: '40px 48px' }}>
      <div style={{ textAlign: 'center', borderBottom: '2px solid #1a1a1a', paddingBottom: '18px', marginBottom: '22px' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'center', flexWrap: 'wrap', gap: '0 0' }}><ContactLine cv={cv} color="#555" /></div>
      </div>
      ...
      {cv.publications?.length && <><SH_AC>Publications</SH_AC><ul style={{ paddingLeft: '16px', margin: 0 }}>{cv.publications.map((p, i) => <li key={i} style={{ fontSize: '10.5pt', lineHeight: 1.6, marginBottom: '4px' }}>{p}</li>)}</ul></>}
