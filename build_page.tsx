...
const DID_YOU_KNOWS = [
  ...
  { fact: 'The ideal CV length for most professionals is 1–2 pages.', tip: 'A tight, focused CV signals confidence and clarity.' },
...
export default function BuildPage() {
  ...
  const [error, setError] = useState<{ title: string; msg: string; type: 'payment' | 'input' | 'server' | 'network' | 'file' } | null>(null)
  ...
  function validate(): string | null {
    ...
    if (needsJD) {
      const m = inputMethod === 'paste' ? jdMethod : jd3Method
      if (m === 'paste') {
        const jd = inputMethod === 'paste' ? refs.jdPaste.current?.value.trim() : refs.jd3Paste.current?.value.trim()
        if (!jd) return 'jd'
      } else {
        const f = inputMethod === 'paste' ? uploadedJD : uploadedJD3
        if (!f) return 'jdfile'
      }
    }
    return null
  ...
  async function handleGenerate() {
    ...
    if (validErr) {
      const msgs: Record<string, {title:string;msg:string;type:any}> = {
        phone:   { title: 'Phone number required', msg: 'Please enter your phone number to continue.', type: 'input' },
        ...
        jdfile:  { title: 'No job description file', msg: 'Please upload the job description before generating.', type: 'input' },
  ...
  async function doGenerate(normalizedPhone: string) {
    try {
      ...
      if (inputMethod === 'paste') {
        if (pasteMethod === 'upload' && uploadedCV) {
          try {
            rawContent = await extractFile(uploadedCV)
          } catch (err: any) {
            setIsGenerating(false)
            setError({ title: 'Could not read your file', msg: err.message || 'Please try a different file or paste your CV as text.', type: 'file' })
            return
          }
        } else {
        ...
        if (needsJD) {
          if (jdMethod === 'upload' && uploadedJD) {
            try {
              jobDescription = await extractFile(uploadedJD)
            } catch (err: any) {
              setIsGenerating(false)
              setError({ title: 'Could not read job description file', msg: err.message || 'Please try pasting it as text instead.', type: 'file' })
              return
            }
          } else {
            jobDescription = refs.jdPaste.current?.value || ''
          }
        }
      }
      ...
      if (inputMethod === 'form') {
        ...
        if (needsJD) {
          if (jd3Method === 'upload' && uploadedJD3) {
            try {
              jobDescription = await extractFile(uploadedJD3)
            } catch (err: any) {
              setIsGenerating(false)
              setError({ title: 'Could not read job description file', msg: err.message || 'Please try pasting it as text instead.', type: 'file' })
              return
            ...
            jobDescription = refs.jd3Paste.current?.value || ''
          }
        }
      }
      ...
      if (!data.success) {
        ...
        if (data.error === 'NO_CREDITS') {
          setError({ title: 'No credits', msg: 'No credits found. Please complete payment first.', type: 'payment' })
        } else if (res.status === 503) {
          setError({ title: 'Service busy', msg: data.error || 'AI is busy right now. Wait 30 seconds and try again.', type: 'server' })
        } else {
      ...
      setIsGenerating(false)
      setError({ title: 'Network error', msg: 'Lost connection mid-generation. Please try again.', type: 'network' })
    }
  }
  ...
  function triggerPaystack(normalizedPhone: string) {
    ...
    script.onload = () => {
      const handler = (window as any).PaystackPop.setup({
        ...
        onClose: () => setError({ title: 'Payment cancelled', msg: 'Payment was not completed.', type: 'payment' })
      })
      handler.openIframe()
    }
    ...
    <div style={{ minHeight: '100vh', background: 'linear-gradient(160deg, #f0fdf9 0%, #f8fafc 40%, #fefdfb 100%)' }}>
      ...
      {step === 1 && (
        <div style={{ maxWidth: '900px', margin: '0 auto', padding: '64px 28px 80px' }}>
          ...
          <h1 style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 'clamp(2rem, 5vw, 3rem)', fontWeight: 600, color: '#0a0f1a', marginBottom: '10px', lineHeight: 1.05, letterSpacing: '-0.5px' }}>What do you need today?</h1>
          <p style={{ fontSize: '15px', color: '#64748b', marginBottom: '48px', fontWeight: 300, lineHeight: 1.7 }}>Choose the document type below. One click and we guide you through the rest.</p>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(260px, 1fr))', gap: '16px', marginBottom: '36px' }}>
            {CV_TYPES.map((t, idx) => (
              <div key={t.id} onClick={() => { setCvType(t.id); go(2) }}
                style={{ background: 'white', border: '1px solid #e2e8f0', borderRadius: '20px', padding: '28px 24px', cursor: 'pointer', transition: 'all 0.25s', position: 'relative', overflow: 'hidden', boxShadow: '0 2px 12px rgba(0,0,0,0.04)' }}
              ...
              >
                ...
                <div style={{ borderTop: '1px solid #f1f5f9', paddingTop: '14px' }}>
                  <div style={{ fontSize: '12px', color: '#94a3b8', marginBottom: '14px' }}><span style={{ fontWeight: 500, color: '#475569' }}>Best for: </span>{t.best}</div>
          ...
          <div style={{ display: 'flex', gap: '12px', padding: '16px 20px', background: '#f0fdf9', border: '1px solid rgba(13,148,136,0.2)', borderRadius: '14px', alignItems: 'flex-start' }}>
            ...
            <p style={{ fontSize: '13.5px', color: '#475569', lineHeight: 1.65, fontWeight: 300 }}><strong style={{ fontWeight: 600, color: '#0a0f1a' }}>Not sure which to pick?</strong> Choose Professional CV — it works for most job applications.</p>
          </div>
        </div>
      )}
      ...
      {step === 2 && (
        <div style={{ maxWidth: '860px', margin: '0 auto', padding: '64px 28px 80px' }}>
          ...
          <h1 style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 'clamp(1.8rem, 5vw, 2.8rem)', fontWeight: 600, color: '#0a0f1a', marginBottom: '10px', lineHeight: 1.05 }}>How would you like to share your details?</h1>
          <p style={{ fontSize: '15px', color: '#64748b', marginBottom: '48px', fontWeight: 300, lineHeight: 1.7 }}>Both options give the same quality result.</p>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '20px', marginBottom: '32px' }}>
            {[
              { method: 'paste' as const, icon: '📋', title: 'Share my old CV', desc: 'Paste the text or upload your existing CV file. Our AI reads and rebuilds everything.', time: '⚡ Under 1 minute', sub: 'Fastest option' },
              { method: 'form' as const,  icon: '📝', title: 'Fill a guided form', desc: "We walk you through each section with clear instructions and examples.", time: '⚡ Under 3 minutes', sub: 'Great if no old CV' },
            ].map(opt => (
              <div key={opt.method} onClick={() => { setInputMethod(opt.method); go(3) }}
      ...
      {step === 3 && inputMethod === 'paste' && (
        <div style={{ maxWidth: '640px', margin: '0 auto', padding: '64px 28px 80px' }}>
          ...
          <p style={{ fontSize: '14px', color: '#64748b', marginBottom: '36px', fontWeight: 300, lineHeight: 1.7 }}>Paste your CV text or upload a PDF/image. The messier the better — our AI organises everything.</p>
          ...
          {pasteMethod === 'paste' ? (
            <Card title="Paste your CV, notes, or anything you have" desc="Copy from Word, PDF, WhatsApp, email — any source." tip="💡 Don't worry about formatting. Rough notes work perfectly.">
              <textarea ref={refs.paste} style={TA(220)} rows={12} placeholder={`Paste your CV content here...\n\nExample:\nKwame Mensah\nIT Officer at MTN Ghana 2019-present\nBefore that Vodafone IT support 2017-2019\nBSc Computer Science KNUST 2016`} />
            </Card>
          ) : (
            <Card title="Upload your CV file" desc="We accept PDF, JPG, PNG, and screenshots. Our AI reads any format.">
              <UploadZone label="Drop your CV here or click to browse" hint="PDF · Image (JPG, PNG, screenshot) · Max 10MB" onFile={setUploadedCV} file={uploadedCV} />
              <div style={{ marginTop: '12px', padding: '10px 14px', background: '#fffbeb', border: '1px solid #fde68a', borderRadius: '10px', fontSize: '12px', color: '#854d0e', lineHeight: 1.6 }}>
                <strong>Word documents (.docx)?</strong> Save as PDF first, or copy-paste your content into "Paste text" instead.
              </div>
            </Card>
          )}
          ...
          {needsJD && (
            <div style={{ background: '#fffbf5', border: '2px dashed #f59e0b', borderRadius: '18px', padding: '24px', marginBottom: '16px' }}>
              <div style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: '1.3rem', fontWeight: 600, color: '#0a0f1a', marginBottom: '4px', display: 'flex', alignItems: 'center', gap: '10px', flexWrap: 'wrap' }}>
                🎯 Job Description
                <span style={{ fontSize: '10px', fontWeight: 600, background: '#fef9c3', color: '#854d0e', padding: '3px 10px', borderRadius: '20px', fontFamily: "'DM Sans', sans-serif" }}>Required for {cvType === 'cover_letter' ? 'Cover Letter' : 'Targeted CV'}</span>
              </div>
              <p style={{ fontSize: '13px', color: '#64748b', lineHeight: 1.65, marginBottom: '14px', fontWeight: 300 }}>Paste the job posting or upload a screenshot.</p>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px', marginBottom: '14px' }}>
                {[{id:'paste',label:'✎ Paste text'},{id:'upload',label:'↑ Upload file'}].map(opt => (
                  <button key={opt.id} onClick={() => setJdMethod(opt.id as any)} style={{ padding: '12px', border: `2px solid ${jdMethod === opt.id ? '#0d9488' : '#e2e8f0'}`, borderRadius: '12px', background: jdMethod === opt.id ? '#f0fdf9' : 'white', color: jdMethod === opt.id ? '#0f766e' : '#64748b', fontWeight: jdMethod === opt.id ? 700 : 500, fontSize: '13px', cursor: 'pointer', fontFamily: "'DM Sans', sans-serif" }}>
                    {opt.label} {jdMethod === opt.id ? '✓' : ''}
                  </button>
                ))}
              </div>
              {jdMethod === 'paste' ? (
                <textarea ref={refs.jdPaste} style={TA(120)} rows={6} placeholder="Paste job posting or describe the role..." />
              ) : (
                <UploadZone label="Upload the job posting" hint="PDF · Image (screenshot, WhatsApp forward)" onFile={setUploadedJD} file={uploadedJD} />
              )}
            </div>
          )}
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: '24px', gap: '12px', flexWrap: 'wrap' }}>
            ...
            <button onClick={handleGenerate} disabled={isGenerating || extractingFile} style={{ ...btnPrimary, opacity: (isGenerating || extractingFile) ? 0.6 : 1 }}>
              {extractingFile ? 'Reading file...' : isGenerating ? 'Generating...' : `Generate my ${cvType === 'cover_letter' ? 'cover letter' : 'CV'} →`}
            </button>
          </div>
        </div>
      )}
      ...
      {step === 3 && inputMethod === 'form' && (
        <div style={{ maxWidth: '900px', margin: '0 auto', padding: '64px 28px 80px' }}>
          ...
          <p style={{ fontSize: '14px', color: '#64748b', marginBottom: '40px', fontWeight: 300, lineHeight: 1.7 }}>Fill in each section. Optional sections can be skipped.</p>

          <SectionCard num="Section 1 of 5" title="Basic Details" desc="Fields marked * are required.">
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '14px', marginBottom: '12px' }}>
              <Field label="Full Name *" inputRef={refs.fullName} placeholder="e.g. Kwame Mensah" />
              <Field label="Phone Number *" inputRef={refs.phone} placeholder="e.g. 0551234567" />
              <Field label="Email Address *" inputRef={refs.email} placeholder="e.g. kwame@gmail.com" />
              <Field label="Location *" inputRef={refs.location} placeholder="e.g. Accra, Ghana" />
            </div>
            <Divider label="Optional" />
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '14px' }}>
              <Field label="Nationality" inputRef={refs.nationality} placeholder="e.g. Ghanaian" />
              <Field label="Date of Birth" inputRef={refs.dob} placeholder="e.g. 14 March 1995" />
              <Field label="LinkedIn URL" inputRef={refs.linkedin} placeholder="e.g. linkedin.com/in/kwame" />
              <Field label="Languages Spoken" inputRef={refs.languages} placeholder="e.g. English, Twi" />
            </div>
          </SectionCard>

          <SectionCard num="Section 2 of 5" title="Education" desc="From SHS to your highest qualification.">
            <Tip text="💡 List as shown — any order is fine, our AI organises it." />
            <ExBox text={`University of Cape Coast – MSc Agriculture – 2022-2024\nUniversity of Ghana – BSc Computer Science – 2018-2022\nSt Thomas Aquinas SHS – WASSCE – 2013-2016`} />
            <textarea ref={refs.education} style={TA(110)} rows={5} placeholder="Write your education history..." />
          </SectionCard>

          <SectionCard num="Section 3 of 5" title="Work Experience" desc="All workplaces — including internships, national service, volunteer roles.">
            <Tip text="💡 No need to list duties — our AI generates achievement bullets automatically." />
            <ExBox text={`Staff Nurse – Ridge Hospital – 2019-Present\nIT Officer – MTN Ghana – 2017-2019\nNational Service – GRA, Kumasi – 2016-2017`} />
            <textarea ref={refs.experience} style={TA(110)} rows={5} placeholder="Write your work experience..." />
          </SectionCard>

          <SectionCard num="Section 4 of 5" title="Additional Information" desc="Awards, certifications, memberships, leadership roles." opt>
            <Tip text="💡 Skip if nothing applies." />
            <ExBox text={`President – Student Council, KNUST – 2022\nGoogle Analytics Certified – 2023`} />
            <textarea ref={refs.additionalInfo} style={TA(90)} rows={4} placeholder="Or leave blank to skip..." />
          </SectionCard>

          <SectionCard num="Section 5 of 5" title="Help us get it right" desc="Corrections, things to emphasise or remove." opt>
            <ExBox text={`"Promoted to Senior Manager in 2022 – please add this"\n"Please emphasise leadership"`} />
            <textarea ref={refs.specialRequests} style={TA(90)} rows={4} placeholder="Or leave blank to skip..." />
          </SectionCard>
          ...
          {needsJD && (
            <div style={{ background: '#fffbf5', border: '2px dashed #f59e0b', borderRadius: '18px', padding: '24px', marginBottom: '16px' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px', marginBottom: '14px' }}>
                {[{id:'paste',label:'✎ Paste text'},{id:'upload',label:'↑ Upload file'}].map(opt => (
                  <button key={opt.id} onClick={() => setJd3Method(opt.id as any)} style={{ padding: '12px', border: `2px solid ${jd3Method === opt.id ? '#0d9488' : '#e2e8f0'}`, borderRadius: '12px', background: jd3Method === opt.id ? '#f0fdf9' : 'white', color: jd3Method === opt.id ? '#0f766e' : '#64748b', fontWeight: jd3Method === opt.id ? 700 : 500, fontSize: '13px', cursor: 'pointer', fontFamily: "'DM Sans', sans-serif" }}>
                    {opt.label} {jd3Method === opt.id ? '✓' : ''}
                  </button>
                ))}
              </div>
              {jd3Method === 'paste' ? (
                <textarea ref={refs.jd3Paste} style={TA(120)} rows={6} placeholder="Paste job posting..." />
              ) : (
                <UploadZone label="Upload the job posting" hint="PDF · Image (screenshot, WhatsApp forward)" onFile={setUploadedJD3} file={uploadedJD3} />
              )}
            </div>
          )}
          ...
          <div style={{ width: '100%', maxWidth: '540px', textAlign: 'center' }}>
            <div style={{ position: 'relative', width: '100px', height: '100px', margin: '0 auto 32px' }}>
              <svg width="100" height="100" viewBox="0 0 100 100" style={{ transform: 'rotate(-90deg)' }}>
                ...
                <circle cx="50" cy="50" r="44" fill="none" stroke="url(#tealGrad)" strokeWidth="5" strokeLinecap="round" strokeDasharray={`${2 * Math.PI * 44}`} strokeDashoffset={`${2 * Math.PI * 44 * (1 - loadingPct / 100)}`} style={{ transition: 'stroke-dashoffset 0.6s ease' }} />
            ...
            <h2 style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 'clamp(1.6rem, 5vw, 2.2rem)', color: 'white', fontWeight: 500, marginBottom: '8px', lineHeight: 1.2 }}>{LOADING_STEPS[loadingStep]?.msg || 'Building your CV...'}</h2>
...
const TA = (minH: number): React.CSSProperties => ({ width: '100%', padding: '14px 16px', border: '1.5px solid #e2e8f0', borderRadius: '12px', fontFamily: "'DM Sans', sans-serif", fontSize: '13.5px', color: '#0a0f1a', resize: 'none', lineHeight: 1.65, minHeight: `${minH}px`, transition: 'border-color 0.2s' })
const btnPrimary: React.CSSProperties = { display: 'inline-flex', alignItems: 'center', gap: '10px', padding: '15px 40px', background: 'linear-gradient(135deg, #0d9488, #0f766e)', border: 'none', borderRadius: '50px', fontSize: '14.5px', fontWeight: 600, color: 'white', cursor: 'pointer', letterSpacing: '0.3px', boxShadow: '0 8px 28px rgba(13,148,136,0.35)', transition: 'all 0.2s' }
const btnBack: React.CSSProperties = { display: 'inline-flex', alignItems: 'center', gap: '6px', padding: '14px 24px', background: 'white', border: '1px solid #e2e8f0', borderRadius: '50px', fontSize: '13.5px', fontWeight: 500, color: '#475569', cursor: 'pointer' }
...
function Tip({ text }: { text: string }) { return <div style={{ background: '#f0fdf9', borderLeft: '3px solid #0d9488', borderRadius: '0 8px 8px 0', padding: '10px 14px', fontSize: '12.5px', color: '#475569', lineHeight: 1.65, marginBottom: '12px', fontWeight: 300 }}>{text}</div> }
...
function Divider({ label }: { label: string }) {
  return (<div style={{ display: 'flex', alignItems: 'center', gap: '10px', margin: '16px 0 14px', fontSize: '11px', fontWeight: 500, color: '#94a3b8' }}><div style={{ flex: 1, height: '1px', background: '#e2e8f0' }} /><span>{label}</span><div style={{ flex: 1, height: '1px', background: '#e2e8f0' }} /></div>)
}

function Field({ label, placeholder, inputRef }: { label: string; placeholder: string; inputRef: React.RefObject<HTMLInputElement> }) {
  return (<div style={{ display: 'flex', flexDirection: 'column', gap: '5px' }}><label style={{ fontSize: '12.5px', fontWeight: 500, color: '#334155' }}>{label}</label><input ref={inputRef} placeholder={placeholder} style={{ padding: '12px 14px', border: '1.5px solid #e2e8f0', borderRadius: '12px', fontFamily: "'DM Sans', sans-serif", fontSize: '13.5px', color: '#0a0f1a', transition: 'border-color 0.2s' }} /></div>)
}
...
function PhoneAndPrice({ phone, setPhone }: { phone: string; setPhone: (v: string) => void }) {
  return (<>
    <div style={{ background: 'white', border: '1px solid #e2e8f0', borderRadius: '18px', padding: '24px', marginBottom: '16px', boxShadow: '0 1px 8px rgba(0,0,0,0.04)' }}>
      ...
      <p style={{ fontSize: '13px', color: '#64748b', marginBottom: '12px', fontWeight: 300, lineHeight: 1.65 }}>Your credits are linked to your phone number. Returning users skip payment.</p>
      <input value={phone} onChange={e => setPhone(e.target.value)} placeholder="e.g. 0551234567 or +233551234567" style={{ width: '100%', padding: '13px 16px', border: '1.5px solid #e2e8f0', borderRadius: '12px', fontFamily: "'DM Sans', sans-serif", fontSize: '13.5px', color: '#0a0f1a' }} />
    </div>
...
function ErrorDisplay({ error, onRetry, onDismiss }: { error: any; onRetry: () => void; onDismiss: () => void }) {
  ...
  const icons: Record<string, string> = { payment: '💳', input: '📋', server: '⚙️', network: '🌐', file: '📄' }
  const colors: Record<string, string> = { payment: '#f59e0b', input: '#3b82f6', server: '#ef4444', network: '#8b5cf6', file: '#ef4444' }
  const bgColors: Record<string, string> = { payment: '#fffbeb', input: '#eff6ff', server: '#fef2f2', network: '#f5f3ff', file: '#fef2f2' }
      ...
      <div style={{ display: 'flex', gap: '14px', alignItems: 'flex-start' }}>
        ...
        <div style={{ flex: 1 }}>
          ...
          <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
            ...
            <a href="https://wa.me/233559519783?text=Hi,%20I%20need%20help" target="_blank" rel="noopener noreferrer" style={{ padding: '8px 18px', background: '#dcfce7', color: '#166534', borderRadius: '50px', fontSize: '12.5px', fontWeight: 600, textDecoration: 'none' }}>💬 WhatsApp Support</a>
          </div>
        </div>
      </div>
    </div>
...
function UploadZone({ label, hint, onFile, file }: { label: string; hint: string; onFile: (f: File | null) => void; file: File | null }) {
  ...
  if (file) return (
    <div style={{ display: 'flex', alignItems: 'center', gap: '14px', padding: '14px 16px', background: '#f0fdf9', border: '1.5px solid #0d9488', borderRadius: '12px' }}>
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ fontSize: '13.5px', fontWeight: 600, color: '#0a0f1a', marginBottom: '2px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{file.name}</div>
  ...
  return (
    ...
    >
      ...
      <input ref={ref} type="file" accept=".pdf,.jpg,.jpeg,.png,.webp" style={{ display: 'none' }} onChange={e => { if (e.target.files?.[0]) onFile(e.target.files[0]) }} />
    </div>
  )
}
