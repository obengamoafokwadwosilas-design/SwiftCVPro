...
function Paginated({ cv, A, config }: { cv: GeneratedCV; A: string; config: TemplateConfig }) {
  ...
  const sideSlice = (pageIndex: number): React.ReactNode =>
    config.buildSidebarBlocks
      ...
      : undefined
  // A fixed element in Chrome print is clipped to the printable box, so a
  // sidebar cannot bleed through a negative left page margin. Tandem leaves
  // the left margin at zero and reserves the sidebar's column inside the flow.
  ...
  return (
    <aside style={{ padding: '30px 20px 24px', color: '#fff', fontFamily: BODY_SERIF, WebkitPrintColorAdjust: 'exact', printColorAdjust: 'exact' }}>
      ...
      {education.length > 0 && <div style={{ paddingBottom: 18, marginBottom: 18, borderBottom: '1px solid rgba(255,255,255,0.28)' }}>
        {label('Education')}
        {education.map((item, index) => <div key={index} style={{ marginBottom: index === education.length - 1 ? 0 : 13, color: 'rgba(255,255,255,0.92)' }}><div style={{ fontSize: 13.5, fontWeight: 700, lineHeight: 1.32 }}>{item.qualification}{item.field ? ` in ${item.field}` : ''}</div><div style={{ fontSize: 12.8, lineHeight: 1.36, color: 'rgba(255,255,255,0.68)', marginTop: 3 }}>{item.institution}</div>{(item.endYear || item.startYear) && <div style={{ fontSize: 12, letterSpacing: 0.5, color: 'rgba(255,255,255,0.5)', marginTop: 4 }}>{[item.startYear, item.endYear].filter(Boolean).join(' – ')}</div>}</div>)}
      </div>}
      {cv.skills?.length ? <div style={{ paddingBottom: references.length ? 18 : 0, marginBottom: references.length ? 18 : 0, borderBottom: references.length ? '1px solid rgba(255,255,255,0.28)' : undefined }}>
        {label('Skills')}
        {cv.skills.map((skill, index) => <div key={index} style={{ display: 'flex', gap: 6, fontSize: 13, lineHeight: 1.35, color: 'rgba(255,255,255,0.9)', marginBottom: index === cv.skills!.length - 1 ? 0 : 7 }}><span aria-hidden="true" style={{ flex: '0 0 auto', color: 'rgba(255,255,255,0.56)' }}>•</span><span>{skill}</span></div>)}
      </div> : null}
      {references.length > 0 && <div>
        {label('References')}
        {references.map((reference, index) => <div key={index} style={{ fontSize: 13, lineHeight: 1.4, color: 'rgba(255,255,255,0.9)', marginBottom: index === references.length - 1 ? 0 : 10 }}>{reference}</div>)}
      </div>}
    </aside>
  )
}
...
// ════════════════════════════════════════════════════════════════
// TEMPLATE PREVIEW — the real template, scaled down
// ════════════════════════════════════════════════════════════════
// Renders each template's ACTUAL Frame/Header/blocks with sample content and
// shrinks it with a CSS transform, instead of a hand-drawn thumbnail. That
// means a preview is always a true picture of the design and updates itself
// whenever a template changes — no thumbnail to maintain by hand.
//
// Deliberately does NOT use the Paginated engine: no measure pass, no layout
// effects, no correction pass. It is plain static JSX, so rendering a dozen of
// them in the picker stays cheap. Content is sized to overflow one page so the
// preview always looks like a full, filled sheet (the overflow is clipped).
const PREVIEW_CV: GeneratedCV = {
  fullName: 'Amara Mensah',
  jobTitle: 'Operations Manager',
  email: 'amara.mensah@email.com',
  phone: '024 555 0198',
  location: 'Accra, Ghana',
  linkedin: 'linkedin.com/in/amaramensah',
  summary: 'Operations leader with over ten years across logistics and financial services, known for turning fragmented processes into dependable systems. Built and led teams of up to twenty-five, managed multi-million cedi budgets, and consistently delivered measurable service improvements.',
  experience: [
    { id: 'e1', role: 'Operations Manager', company: 'Silverline Logistics', startDate: 'Mar 2021', endDate: 'Present', bullets: [
      'Restructured regional dispatch operations, cutting average delivery times by 32% across fourteen routes',
      'Led a twenty-five person team through a full warehouse migration with zero disruption to client service',
      'Negotiated supplier contracts that reduced annual fulfilment costs by GHS 480,000',
    ] },
    { id: 'e2', role: 'Assistant Operations Manager', company: 'Harbour Freight Ghana', startDate: 'Jun 2017', endDate: 'Feb 2021', bullets: [
      'Introduced weekly performance reporting that became the standard across three regional branches',
      'Coordinated inbound and outbound scheduling for a fleet of forty vehicles',
      'Trained sixty staff on revised safety and compliance procedures',
    ] },
    { id: 'e3', role: 'Logistics Coordinator', company: 'Accra Distribution Co.', startDate: 'Jan 2015', endDate: 'May 2017', bullets: [
      'Managed daily route planning and driver allocation for high-volume retail deliveries',
      'Maintained inventory accuracy above 99% through disciplined cycle counting',
    ] },
  ],
  education: [
    { id: 'd1', qualification: 'MBA', field: 'Operations Management', institution: 'University of Ghana Business School', startYear: '2018', endYear: '2020', grade: 'Distinction' },
    { id: 'd2', qualification: 'BSc', field: 'Business Administration', institution: 'KNUST', startYear: '2011', endYear: '2015', grade: 'First Class' },
  ],
  skills: ['Operations Strategy', 'Supply Chain Planning', 'Team Leadership', 'Budget Management', 'Process Improvement', 'Vendor Negotiation', 'Inventory Control', 'Performance Reporting', 'Risk & Compliance', 'Stakeholder Management'],
  attributes: [
    'Keeps calm and decisive under operational pressure.',
    'Builds order from disorder without slowing the team down.',
    'Communicates clearly across drivers, managers and clients.',
    'Follows through until problems are genuinely closed.',
  ],
  languages: ['English (Fluent)', 'Twi (Native)', 'French (Basic)'],
}

export function TemplatePreview({ templateId, width = 92 }: { templateId: TemplateId; width?: number }) {
  const design = TEMPLATE_MAP[templateId] || 'meridian'
  const config = TEMPLATES_CONFIG[design]
  if (!config) return null
  const A = DEFAULT_ACCENT[design] || '#0d9488'
  const scale = width / PAGE_W
  const height = Math.round(PAGE_H * scale)
  const blocks = config.buildBlocks(PREVIEW_CV, A).slice(0, 30)
  const sideBlocks = config.buildSidebarBlocks ? config.buildSidebarBlocks(PREVIEW_CV, A).slice(0, 30) : undefined
  return (
    <div aria-hidden="true" style={{ width, height, overflow: 'hidden', borderRadius: 5, border: '1px solid #e2e8f0', boxShadow: '0 2px 8px rgba(10,15,26,0.13)', background: '#fff', flexShrink: 0, position: 'relative' }}>
      <div style={{ width: PAGE_W, height: PAGE_H, transform: `scale(${scale})`, transformOrigin: 'top left', pointerEvents: 'none' }}>
        <config.Frame
          cv={PREVIEW_CV}
          A={A}
          pageIndex={0}
          sidebarChildren={sideBlocks ? sideBlocks.map(b => <div key={b.key}>{b.node}</div>) : undefined}
        >
